...
**보안**
- 릴리즈 빌드 ProGuard/R8 난독화 적용 (`isMinifyEnabled`, `isShrinkResources`)
- Firebase App Check (Play Integrity)로 비정상 클라이언트 요청 차단
- SMS 인증 API 키를 Cloud Functions Secrets 환경변수로 주입, 코드 하드코딩 없음

**운영 비용 관리**
- 미인증 익명 사용자를 스케줄 함수로 자동 정리해 Firebase Auth/DB 리소스 낭비 방지

<br>

## 향후 개선 방향

- feature 모듈에 흩어진 도메인/데이터 로직을 `:domain`, `:data` Gradle 모듈로 이관
- Cloud Functions 단위 테스트 커버리지 확대
- Compose UI 테스트 및 스크린샷 테스트 도입
